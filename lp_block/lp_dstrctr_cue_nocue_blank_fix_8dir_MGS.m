%lp_dstrctr_cue_nocue_blank_fix_8dir_MGS
...
% allows the user to set the position of the targets (and the cue). It also
% flashes 'probe' memory guided saccade trials.
...
editable( 'reward', 'fix_radius', 'wait_release', 'hold_time', 'pre_hold_time', 'blank_time', 'spec_radius', 'neut_cue', 'invalid_cue', 'probe_trials');
...
probe_trials = 50; % 50% of trials are probe trials


% MGS Variables:
mgs_cue_time = 80;
mgs_delay = 500;
mgs_max_reaction_time = 500;
mgs_saccade_time = 80;
mgs_hold_target_time = 200;
mgs_targ_win = 2;
...
% Probability of being a probe trial
probe_prob = probe_trials / 100;
probe_prob_vals = [0 1];
probe_prob_real = [(1 - probe_prob) probe_prob];
rand_probeprobidx = sum( rand >= cumsum([0,probe_prob_real]) ); 
rand_probeprob = probe_prob_vals( rand_probeprobidx );
bhv_variable( 'probe_prob', rand_probeprob );
...
% Position Probe MGS Trial Cue Where Target would appear
success = reposition_object(mgs_cue, new_targ_xpos, new_targ_ypos);
success = reposition_object(mgs_target, new_targ_xpos, new_targ_ypos);
...
% If Probe Trial, Run MGS Task.
if rand_probeprob
    % cue epoch
    toggleobject(mgs_cue, 'eventmarker', 153); % turn on cue
    ontarget = eyejoytrack('holdfix', start_spot, fix_radius, mgs_cue_time);
    if ~ontarget,
        trialerror(2); % broke fixation
        toggleobject([start_spot mgs_cue], 'eventmarker', 154)
        idle(200, [1, 0, 0]); % Red Error Screen
        return
    end
    toggleobject(mgs_cue, 'eventmarker', 155); % turn off sample

    % delay epoch
    ontarget = eyejoytrack('holdfix', start_spot, fix_radius, mgs_delay);
    if ~ontarget,
        trialerror(2); % broke fixation
        toggleobject(start_spot, 'eventmarker', 156)
        idle(200, [1, 0, 0]); % Red Error Screen
        return
    end

    % response
    toggleobject(start_spot, 'eventmarker', 157); % turns off fixation point
    [ontarget rt] = eyejoytrack('holdfix', start_spot, fix_radius, mgs_max_reaction_time); % rt will be used to update the graph on the control screen
    ...
    ontarget = eyejoytrack('acquirefix', mgs_target, mgs_targ_win, mgs_saccade_time);
    if ~ontarget,
    ...
    ontarget = eyejoytrack('holdfix', mgs_target, mgs_targ_win, mgs_hold_target_time);
    if ~ontarget,
        trialerror(2); % broke fixation
        eventmarker(160); 
        idle(200, [1, 0, 0]); % Red Error Screen
