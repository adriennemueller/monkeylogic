...
% Turn on Targets
toggleobject(targ1, 'eventmarker', 124);
...
held = state(1); fixated = state(2);
if (~held) || (~fixated)
    toggleobject([start_spot cue targ1], 'eventmarker', 125); %Turn off fixation spot and targets
    trialerror(3); % Released too soon or broke fix
    idle(200, [1, 0, 0]); % Red Error Screen
    return
end

toggleobject(targ1, 'eventmarker',126);
...
toggleobject(targ1new, 'eventmarker', 128);
...
if (~fixated)
    toggleobject([start_spot, cue, targ1new], 'eventmarker', 129); %Turn off fixation spot and targets
...
if (~not_released && (comb == 1))
    toggleobject([start_spot, cue, targ1new], 'eventmarker', 130); %Turn off fixation spot and targets
...
if (not_released && (comb == 2))
    toggleobject([start_spot, cue, targ1new], 'eventmarker', 131); %Turn off fixation spot and targets
    trialerror(5); % Did not release in time
    idle(1500, [1, 0, 0]); % Red Error Screen
    return
end

toggleobject([start_spot, cue, targ1new], 'eventmarker', 132); %Turn off fixation spot and targets
