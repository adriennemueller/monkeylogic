...
function gen_mobile_dstrctr_conditions_file_cued_1mag( filename, nTrials )
    path( path, '../' );
	...
	for i = 1:nTrials
		...
		tar1_end   = get_tar1_end(tar1_start);
		tar2       = randi(36)*5;
     
%         tposes = gen_target_positions();
        tposes = gen_static_targets( nTrials, i);
...
function rslt = get_tar1_end( tar1_start )

    mag = 80;
    t1end = tar1_start + mag;
...
function make_condition(t1_s, t1_e, t2_s, t2_e, t1x, t1y, t2x, t2y, cue_angle, cond, block, freq, s, fid)
    ...
    generate_condition(...
        ...
        'Block', block, ...
        'Frequency', freq, ...
        'TimingFile', 'lp_dstrctr_cue_blank_fix', ...
        'Info', s, ...
...
function rslt = gen_static_targets( nTrials, currTrial )
    
    totalpositions = 4;
    block1end = round(nTrials/totalpositions);
    block2end = round(nTrials/totalpositions *2);
    block3end = round( nTrials/totalpositions *3);
    block4end = nTrials;
    
    radius = 3.5; %randi(5); %Get Randum Radius between 1 and 5
    
    % Right
    if (0 < currTrial) && (currTrial <= block1end )
    [t1x, t1y] = pol2cart( (0 * pi/180), radius); %Convert to polar coordinates
    t2x = -t1x; t2y = -t1y;
    rslt = [t1x t1y t2x t2y 0 1];
        
    % Up
    elseif(block1end < currTrial) && (currTrial <= block2end)
    [t1x, t1y] = pol2cart( (90 * pi/180), radius); %Convert to polar coordinates
    t2x = -t1x; t2y = -t1y;
    rslt = [t1x t1y t2x t2y 90 2];
     
    % Left
    elseif(block2end < currTrial) && (currTrial <= block3end)
    [t1x, t1y] = pol2cart( (180 * pi/180), radius); %Convert to polar coordinates
    t2x = -t1x; t2y = -t1y;
    rslt = [t1x t1y t2x t2y 180 3];
    
    else
    [t1x, t1y] = pol2cart( (270 * pi/180), radius); %Convert to polar coordinates
    t2x = -t1x; t2y = -t1y;
    rslt = [t1x t1y t2x t2y 270 4];    
    end
