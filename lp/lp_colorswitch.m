...
toggleobject([cue targ1], 'eventmarker', 121);

held = eyejoytrack('holdtouch', 1, [], hold_time);
if ~held,
    % flip red
    trialerror(2); % released too early
    toggleobject([cue targ1], 'eventmarker', 126); %Released too soon
    idle(200, [1, 0, 0]);
    return
end
...
if (released && targ1 == targ2)
    % flip red
    trialerror(3); 
    toggleobject([cue targ2], 'eventmarker', 127); % Released when should not have
    idle(200, [1, 0, 0]);
    return
end
...
if (~released && targ1 ~= targ2)
    toggleobject([cue targ2], 'eventmarker', 128 );
    % flip red
...
toggleobject( [cue targ2], 'eventmarker', 124); % Performed Correctly
