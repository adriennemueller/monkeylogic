...
% not change, the animal must keep the lever held to be rewarded. A blue
% distractor is presented simultaneously with the target.
...
% Switch 'fixation spot' for Target and Distractor
toggleobject([start_spot targ1 targblue], 'eventmarker', 121); %Target and Distractor On

% Tests lever remains pressed
held = eyejoytrack('holdtouch', 1, [], hold_time);
if ~held,
    toggleobject([targ1 targblue], 'eventmarker', 126); %Turn off target and distractor
...
if (released && targ1 == targ2)
    toggleobject([targ2 targblue], 'eventmarker', 127); %Turn off target and distractor
...
if (~released && targ1 ~= targ2)
    toggleobject([targ2 targblue], 'eventmarker', 128); %Turn off target and distractor
    trialerror(4); % Did not release in time
    idle(200, [1, 0, 0]); % Red Error Screen
    return % END OF TRIAL
end

toggleobject([targ2 targblue], 'eventmarker', 124); %Turn off target and distractor
