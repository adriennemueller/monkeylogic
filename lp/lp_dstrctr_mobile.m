%lp_dstrctr_mobile (timing script)
...
theta = 359 * rand(1,1); %Get Random Angle
%rho   =   4 * rand(1,1) +1; %Get Randum Radius between 1 and 5
rho = 5;
[new_targ_xpos, new_targ_ypos] = pol2cart(theta, rho); %Convert to polar coordinates

success = reposition_object(targ1, new_targ_xpos, new_targ_ypos);
success = reposition_object(targ2, new_targ_xpos, new_targ_ypos);

%Mirror Distractor to Opposing Coordinates
success = reposition_object(dist1, (-1 * new_targ_xpos), (-1 * new_targ_ypos));
success = reposition_object(dist2, (-1 * new_targ_xpos), (-1 * new_targ_ypos));
