...
% fixed 9/11/14 -WA (to work in JAVA+ mode)
...
elseif ismember(gcbo, get(fig, 'children')),
    ...
    switch callertag,
        ...
        case 'startcal',
            ...
            maxduration = 60000; %continue in 60 seconds if no input
            u = get(gca, 'userdata');
            xs = u(1);
            ys = u(2);
            pixperdeg = u(3);
            ...
            if ~ScreenInfo.IsActive, %initialize I/O and Video
                try
                    disp('<<< MonkeyLogic >>> Signal Calibration >> Initializing Video')
                    mlvideo('init');
                    ...
                    disp('<<< MonkeyLogic >>> Signal Calibration >> Video Initialization Complete')
            ...
            FixXsize = xis;
            FixYsize = yis;
            disp('<<< MonkeyLogic >>> Signal Calibration >> Initialized Targets')
            ...
            disp('<<< MonkeyLogic >>> Signal Calibration >> Starting Calibration')
            while quitflag == 0,
